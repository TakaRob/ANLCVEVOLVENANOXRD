Evaluate the submitted algorithm against the holdout validation set of 119 spatially-binned XRD images.

## Holdout data directory contents

- **`bin_annotations.json`**: Ground truth annotations for 116 bins that contain peaks. Format:
  ```json
  {
    "0_0": {
      "(001)": [[842, 972], [855, 557]],
...
- **`empty_bins.json`**: A list of 3 bin keys that were reviewed and confirmed to contain **no peaks** (e.g., `["20_12", "25_22", "27_16"]`). A correct algorithm should return zero detections for these bins.
...
All bin images are pre-computed in a single HDF5 file at **`/home/takaji/xrd_5x5_bins.h5`**. Load any bin with:
...
1. For each of the 116 bins in `bin_annotations.json`, plus the 3 bins in `empty_bins.json` (119 total):
   a. Load the bin image from `/home/takaji/xrd_5x5_bins.h5`.
...
4. **Aggregate metric**: The primary metric is the **mean F1 score** across all 119 validation bins, weighted equally.
...
Loop over all 119 bins, collect per-bin F1 scores, and compute the mean.

## Important notes

- The holdout bins span a variety of signal levels: some have many strong peaks, some have a single faint peak, and 3 have no peaks at all.
- Total ground truth points across all holdout bins: **368 points** in 116 bins with peaks.
- Average ~3.2 points per non-empty bin, but this varies widely (1–8+ per bin).
