...
H5_PATH = Path("/home/takaji/xrd_3x3_bins.h5")
...
def load_features():
    catalog_path = RESULTS_DIR / "feature_catalog_3x3.json"
    filtered_csv = RESULTS_DIR / "filtered_peaks_3x3.csv"
...
class _ExpansionWorker(QThread):
    ...
    def run(self):
        from spatial_feature_analysis import detect_peaks_with_intensity, _best_per_bin
...
class FeatureViewer(QMainWindow):
    ...
    def _load_data(self):
        ...
        import tifffile
        self._tth_map = tifffile.imread(str(HOLDOUT_DIR / "tth.tiff")).astype(np.float64)
        ...
        ref_mod = load_module(HOLDOUT_DIR / "reflections.py")
        self._ref_degs = ref_mod.degs
        self._ref_labels = ref_mod.deg_labels
        self._show_tth_overlay = False

        from spatial_feature_analysis import estimate_beam_center
    ...
    def _expand_peak_spatially(self, center_row, center_col, seed_peak):
        from spatial_feature_analysis import detect_peaks_with_intensity
    ...
    def _build_explore_feature(self, members, seed_peak):
        from spatial_feature_analysis import _best_per_bin
