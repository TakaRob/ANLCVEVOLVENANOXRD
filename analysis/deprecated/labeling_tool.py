...
BIN_CONFIGS = {
    5: {"h5_path": "/home/takaji/xrd_5x5_bins.h5", "label": "5×5"},
    4: {"h5_path": "/home/takaji/xrd_4x4_bins.h5", "label": "4×4"},
    3: {"h5_path": "/home/takaji/xrd_3x3_bins.h5", "label": "3×3"},
    1: {"h5_path": None, "label": "1×1 (raw)"},
}
...
class LabelingTool(QMainWindow):
    ...
    def _load_project_data(self):
        tth_path = self.project_root / "raw_scans" / "tth.tiff"
        self.tth = tifffile.imread(tth_path).astype(np.float64)
    ...
    def _ensure_raw_grid(self):
        ...
        if hasattr(self, "xrd_files"):
            return
        scan_dir = self.project_root / "raw_scans" / "Scan_0203"
        xrd_dir = scan_dir / "XRD"
        pos_csv = self.project_root / "203 other" / "Scan_0203_position.csv"
        self.xrd_files, self.xrd_file_map, self.n_total = load_xrd_metadata(xrd_dir)
    ...
    def _load_bin_data_for_size(self, bin_size):
        ...
        conf = BIN_CONFIGS.get(bin_size, {})
        self.bins_h5_path = conf.get("h5_path")
        ...
        self._annotations_path = (
            self.project_root / "results" / "scan203" / f"bin_annotations_{bin_size}x{bin_size}.json")
        ...
        if self._annotations_path.exists():
            ...
            old_path = self.project_root / "results" / "scan203" / "bin_annotations.json"
    ...
    def _update_holdout_set(self):
        ...
        bs = self.bin_size
        holdout_dir = self.project_root / f"cvevolve_{bs}x{bs}" / "holdout_data"
        if not holdout_dir.exists():
            QMessageBox.warning(self, "Error", f"Holdout directory not found:\n{holdout_dir}")
            return
...
def main():
    ...
    app.setStyle("Fusion")
    window = LabelingTool(project_root=args.project_root)
    window.show()
    sys.exit(app.exec_())
