...
**Spatial binning.** Rather than analyzing each single-exposure frame independently (which is noisy), nearby frames are grouped into spatial bins and summed. A **3×3 binning** groups 3 rows × 3 columns of adjacent grid positions, summing all ~9 frames in each bin into one image. This reduces noise while preserving spatial variation across the sample. There are **3253 bins** total (52 bin-rows × 74 bin-cols), each producing a 1062×1028 pixel image.
...
All 3253 bin images are pre-computed and stored in a single HDF5 file:

**`/home/takaji/xrd_3x3_bins.h5`**
...
def load_bin_image(bin_key, bins_h5_path="/home/takaji/xrd_3x3_bins.h5"):
...
```python
with h5py.File("/home/takaji/xrd_3x3_bins.h5", "r") as f:
    bin_keys = list(f.keys())  # 3253 keys like "0_0", "0_1", ...
```
...
- `--bins-h5`: Path to the pre-built bin images HDF5 (default: `/home/takaji/xrd_3x3_bins.h5`)
...
- Loading is fast: `h5py.File('/home/takaji/xrd_3x3_bins.h5')['3_7'][:]` takes milliseconds. No need to cache.
