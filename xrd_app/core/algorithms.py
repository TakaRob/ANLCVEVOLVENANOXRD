...
def fit_all_models(tth_centers, radial_profile, valid_mask, tth_min, tth_max):
    ...
    if best_fourier: results["fourier"] = {"profile": best_fourier["profile"], "params": dict(cutoff=best_fourier["cutoff"])}
    return results

def build_background_image(tth_map, tth_centers, radial_profile, bin_indices_2d, tth_edges=None):
