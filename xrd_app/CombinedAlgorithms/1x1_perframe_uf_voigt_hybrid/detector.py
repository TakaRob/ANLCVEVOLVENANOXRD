"""
Per-Frame Detection + Union-Find Linking + Aperture-Probed Voigt Verification.

MATERIALLY NEW APPROACH:
1. True per-frame detection across ALL neighboring frames - the first candidate
   to properly implement the 3-stage pipeline recommended by the task.
2. Shared radial background: compute median background from the sum-stack once,
   then subtract the per-frame share from each frame.
3. Pre-computed per-band noise thresholds from a representative frame, applied
   to all frames. Eliminates expensive per-frame per-band median computation.
4. Union-Find spatial linking to cluster detections across frames.
5. Aperture-probed Voigt profile verification across ALL frames.
6. Hybrid: local sum-stack (radius=2) catches peaks too faint for single frames.
7. Center-frame position refinement for better GT matching.
"""
