"""
Tuned Per-Frame Detection + Union-Find Linking + Aperture-Probed Voigt Verification.

Tuned from r010-8db2d112 (F2=0.467). Key targeted changes:
1. Per-frame radial background subtraction instead of shared sum-stack bg
   - Each frame gets its own radial median background, providing better
     noise estimation per frame rather than averaging across all frames
2. dedup_dist reduced from 25 to 20 — allows more distinct peaks close together
3. refinement_radius increased from 3 to 5 with local stack refinement
   - Uses the local sum-stack for position refinement (less noisy)
4. local_stack_radius increased from 2 to 3 — wider local stack for 
   supplementary detection
5. stack_snr lowered from 3.5 to 3.0 — catches more faint peaks in local stack
6. voigt_r2_thresh lowered from 0.25 to 0.20 — slightly more permissive Voigt
   fit acceptance
"""
...
def detect_fast(tophat_img, cleaned_img, band_data, thresholds,
                ...
                max_detections=30, snr_threshold=2.5):
...
def link_peaks_union_find(all_detections, link_tolerance=7):
    nodes = []
...
def probe_feature(members, raw_frames, bg_per_frame, aperture=4):
    """Probe feature intensity across ALL frames using aperture photometry."""
...
def verify_voigt(net_by_bin, center_bin, voigt_r2_thresh=0.25,
                 ...
                 center_percentile=0.10, io_ratio_thresh=1.15,
    ...
    if n_sig <= 2:
        ...
        # More permissive compact acceptance: center signal above threshold
...
def run_full_pipeline(center_bin, bins_h5_path, tth_map, degs, deg_labels,
                      ...
                      stack_snr=3.0, local_stack_radius=3,
    ...
    # Shared background (computed once from sum-stack)
    ...
    # Stage 1: Per-frame detection with per-frame radial background
    all_det = {}
    for bk, frame in raw_frames.items():
        # Use per-frame radial median for better noise estimation per frame
        frame_bg = compute_radial_bg(frame, tth_data)
        cleaned = frame - frame_bg
    ...
    # Compute local stack cleaned image for position refinement
    ls_cleaned = ls - bg_pf * nl if nl > 0 else None
    ...
    for members in features:
        ...
        # Position refinement using local stack (less noisy than single frame)
        if refinement_radius > 0:
            ref_img = ls_cleaned if nl > 0 else (raw_frames.get(center_bin, None))
            if ref_img is not None:
                if nl == 0:
                    ref_img = ref_img - bg_pf
                r = refinement_radius
