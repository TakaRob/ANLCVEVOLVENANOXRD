...
window, and the same $3\times3$ spatial bins. The independently constructed
territorial map, which bins frames using their true stage coordinates, provided a
common reference. Features were matched across the regular and territorial maps
by reflection, detector position, and overlap among their underlying raw frames.
...
fixed-ROI outline at SNR 4, and an edge profile for each case. The Gaussian curve
in the final column is a fit used to display profile breadth; it is not an extra
acceptance step in the production pipeline. Production xrd-app instead requires
a non-flat, generally decreasing intensity profile after adaptive peak detection
and spatial linking. This distinction matters for the poor case: its spatial
...
Scan\_0203 features. Rows show ideal, median, and poor Gaussian-fit cases selected
at the 90th, 50th, and 10th percentiles of fit $R^2$. Columns show the
true-position territorial reference, the $3\times3$ xrd-app linked shape, the
connected fixed-ROI region at SNR 4, and the SNR dropoff along the fitted major
axis. Orange points are xrd-app detections that follow local Bragg-peak movement;
open blue points use one fixed detector aperture. Orange curves are
visualization-only Gaussian fits, and dashed lines mark the SNR cutoff. The poor
case demonstrates that xrd-app can retain a reproducible outline even when the
intensity profile is not well described by one Gaussian.}
...
retains a linked 16-bin shape. In contrast, the saved xrd-app outline has no
feature-specific outline threshold to tune after detection: a common detector
SNR threshold is followed by local detector-position matching and a spatial
profile consistency test. This combination makes the reconstructed boundary less
sensitive to isolated noisy or missing positions while still allowing imperfect
physical shapes.
...
restrictive detection threshold. Against a true-position territorial reference,
this linked approach remains effective for nonideal profiles for which a fixed
ROI requires feature-specific threshold tuning and can change the inferred area
by severalfold. Applied across three devices, the workflow shows
...
%=====================================================================
The reflection-resolved grain statistics summarized in Section~III~E are shown in
