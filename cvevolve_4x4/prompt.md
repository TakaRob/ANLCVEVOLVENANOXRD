...
**Spatial binning.** Rather than analyzing each single-exposure frame independently (which is noisy), nearby frames are grouped into spatial bins and summed. A **4×4 binning** groups 4 rows × 4 columns of adjacent grid positions, summing all ~16 frames in each bin into one image. This reduces noise while preserving spatial variation across the sample. There are **1859 bins** total (39 bin-rows × 56 bin-cols), each producing a 1062×1028 pixel image.
...
All 1859 bin images are pre-computed and stored in a single HDF5 file:

**`/home/takaji/xrd_4x4_bins.h5`**
...
```python
with h5py.File("/home/takaji/xrd_4x4_bins.h5", "r") as f:
    bin_keys = list(f.keys())  # 1859 keys like "0_0", "0_1", ...
```
...
- `--bins-h5`: Path to the pre-built bin images HDF5 (default: `/home/takaji/xrd_4x4_bins.h5`)
...
- Loading is fast: `h5py.File('/home/takaji/xrd_4x4_bins.h5')['3_7'][:]` takes milliseconds. No need to cache.
